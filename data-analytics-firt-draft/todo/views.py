...
from todo.forms import TodoForm, Addbook
from todo.models import ToDo,Book

import pandas as pd

def dataframe(request):
    df = pd.read_csv(r'/home/flintin/Desktop/todolist/data.csv')
    df1 = df.head(10)
    df2 = df1[['Unique Key','Created Date','Closed Date','Complaint Type','Borough','Latitude','Longitude']].copy()

    grp = df2.groupby (['Complaint Type'])['Unique Key'].count()
    grp = grp.reset_index()

    return render(request, 'dataframetest.html', 
        {'dataframe': df2.to_html(classes="table table-striped"),
        'xvalue': list(grp['Complaint Type'].values),
        'yvalue': list(grp['Unique Key'].values)    
         } )
...
def listbooks(request):
    ...
    try:
        ...
        pg = paginator.page(paginator.num_pages)
    return render(request, "listbooks.html", {"all_books":all_books, "posts":pg})

    # item = Book.objects.get(pk=1)
    # return render(request, 'name.html', {'form': form, 'imgurl':item.pic.url})


#********

def bookdetail(request,id):
    todo = get_object_or_404(Book, id=id)
    return render(request, "bookdetail.html", {"todo":todo})
